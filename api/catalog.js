// Vercel serverless function: builds the full Xtream catalog (with
// precomputed search tokens) for one media kind. Runs on Node, which has a
// far higher CPU/time budget than Cloudflare's free-tier Workers, so this is
// where the "expensive once in a while" catalog build happens instead.
//
// Called by the Cloudflare Worker as:
//   GET /api/catalog?kind=movie|series|channel&host=...&username=...&password=...

const UA = 'IPTVSmartersPro/3.0.0 (Vercel; IPTV Bridge catalog builder)';
...
module.exports = async (req, res) => {
  try {
    const { kind, host: rawHost, username, password } = req.query;
    ...
    return res.status(502).json({ error: err?.message || 'catalog build failed' });
  }
};
